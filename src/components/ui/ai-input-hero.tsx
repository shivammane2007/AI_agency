...
export function HeroWave({ className, style, extendLeftPx = 800, title = "Build with AI.", subtitle = "The AI Fullstack Engineer. Build prototypes, apps, and websites", placeholder = "Describe what you want to create...", buttonText = "Generate", onPromptSubmit }: HeroWaveProps) {
  ...
  useEffect(() => {
    ...
    function createInstancedMaterial() {
      const baseCol = new THREE.Color("#10b981"); // Emerald 500
      const emisCol = new THREE.Color("#059669"); // Emerald 600
    ...
    function initWaveThree() {
      ...
      waveBloomPass = new UnrealBloomPass(new THREE.Vector2(cameraWidth, cameraHeight), 1.8, 0.4, 0.1);
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <h1 className="text-white text-5xl sm:text-7xl lg:text-8xl font-display font-bold tracking-tight drop-shadow-[0_1px_12px_rgba(16,185,129,0.25)] leading-tight">
            {title}
          </h1>
          <p className="text-zinc-400 mt-6 text-lg sm:text-xl md:text-2xl font-light max-w-3xl mx-auto leading-relaxed">
            {subtitle}
          </p>
          <form
          ...
          >
            <div className="relative w-full sm:w-[840px]">
              <div className="relative rounded-2xl p-[1px] bg-gradient-to-br from-emerald-500/20 via-emerald-500/5 to-transparent">
                <textarea
                  value={prompt}
                  onChange={(e) => setPrompt(e.target.value)}
                  placeholder={animatedPlaceholder}
                  rows={5}
                  className="w-full h-32 sm:h-40 resize-none rounded-2xl bg-[rgba(10,10,12,0.7)] border border-white/10 text-white placeholder:text-white/30 outline-none focus:ring-2 focus:ring-emerald-500/40 focus:border-emerald-500/40 backdrop-blur-xl px-6 py-6 pr-20 text-lg shadow-2xl"
                />
              </div>
              <button
                type="submit"
                aria-label={buttonText}
                className="absolute right-4 bottom-4 inline-flex items-center justify-center w-12 h-12 rounded-xl bg-emerald-500 text-black hover:bg-emerald-400 transition-all duration-300 shadow-[0_0_20px_rgba(16,185,129,0.3)]"
              >
