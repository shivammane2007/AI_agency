      ...
      {slicedUsers.map((user, index) => {
        ...
        return (
          <motion.div
            ...
            className="relative cursor-pointer outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary rounded-full transition-shadow hover:shadow-[0_0_15px_rgba(16,185,129,0.3)]"
            style={{
              width: size,
              height: size,
              zIndex,
          ...
          >
            {/* Avatar bubble */}
            <div className="w-full h-full rounded-full overflow-hidden border-2 border-white/10 shadow-md bg-zinc-900">
              {isLengthBubble ? (
                <div className="flex h-full w-full items-center justify-center bg-zinc-800 text-xs font-medium text-white">
            ...
            <AnimatePresence>
              {shouldScale && user.name && (
                ...
                >
                  ...
                  <div className="transform -translate-x-1/2 whitespace-nowrap rounded-md bg-zinc-900 border border-white/10 text-zinc-200 text-xs px-2 py-1 shadow-lg">
                    {user.name}
                  </div>
                </motion.div>
              )}
